...
public class UserDbStorage implements UserStorage {
    ...
    void checkUser(Long id) throws ObjectNotFoundException {
        String sql = "SELECT * FROM USERS WHERE USER_ID = ?";
        List<User> result = jdbcTemplate.query(sql, this::mapToUser, id);
        if (result.isEmpty()) {
            throw new ObjectNotFoundException("User not found");
    ...
    private User mapToUser(ResultSet resultSet, int rowNum) throws SQLException, ObjectNotFoundException {
        User user = new User();
    ...
    private void readFriends(User user) throws ObjectNotFoundException {
        checkUser(user.getId());
        String sql =
        ...
        while (sqlRowSet.next()) {
            user.getFriends().add(sqlRowSet.getLong("id"));
        }
    }

    private void readLikes(User user) throws ObjectNotFoundException {
        checkUser(user.getId());
